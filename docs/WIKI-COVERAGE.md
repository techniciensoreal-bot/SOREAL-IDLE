...
| Wandoos | (non signalé) | **Confirmé, verrouillé** | Test `idle-wiki-table-wandoos` : seuils de speed-cap des 3 OS × 3 difficultés (1e9 / 1e12 / 1e15, 1e21 / 1e27 / 1e33, 1e33 / 1e39 / 1e45), rapports annoncés (×1 000 en Normal, ×1e6 en Evil et Sadistic, ×1e12 entre difficultés pour Wandoos 98) et formules de bonus des 3 OS réévaluées indépendamment. Boot-up, niveaux d'OS et niveaux 98/XL : testés par les tests Wandoos existants. |
| Broken Time Machine | (non signalé) | **Confirmé, verrouillé** | Test `idle-wiki-table-time-machine` : tableau « Machine Speed scaling » (13 niveaux, dont le doublement du niveau 49 au niveau 50 et ×2 500 au niveau 98) et « Highest Boss - 27 » plafonné à 274 (boss 301). Coût du niveau 1 (5 M d'or) et temps de base (1e6 s Normal, 1e18 s Evil) présents dans le moteur. **Non verrouillé, non publié** : effet exact d'un niveau de Gold Multiplier. |
| Energy, Magic, Resource 3 | 6 à 10 | Catalogues vérifiés | Ces pages ne contiennent que des listes de perks, de souhaits et d'objets Sellout : le script `design/compare-catalogs-to-wiki.mjs` (relancé le 2026-09-24) compare les 232 perks, 186 quirks et 231 souhaits au wiki ; il ne reste que des libellés (perks 21, 32, 34, 94 ; quirks 15, 16, 148 ; souhait 202) et **le perk 56 « Macguffin Daycare! »** (5 000 PP, 1 niveau, Evil) volontairement absent du catalogue : son effet n'est pas chiffré, l'ajouter le rendrait achetable sans aucun effet. Plafonds 1E18 / 9E18 avant potions : test `idle-resource-stat-hard-caps`. |
...
Fait le 2026-09-24 : recalcul cellule par cellule avec les fonctions du moteur des tables Wandoos, Time Machine, Boost, Augmentations, Blood Magic, NGU (48 lignes), Hacks (15), Cards (14 types), Gold Diggers (12), Boss Fights (157 lignes) et des zones d'Adventure Mode (32), chacune verrouillée par un test `idle-wiki-table-*` (valeurs du wiki copiées en fixtures ; les tests de la CI ne peuvent pas lire le miroir). Coquilles du wiki consignées dans `NGU-Wiki/external/wiki-table-typos.json`.

1. Tables non recalculées cellule par cellule : Yggdrasil (fruits : coûts et graines vérifiés par l'agent Yggdrasil), Daily Spin (table de récompenses, vérifiée par l'agent), Titans (12 titans vérifiés par l'agent), sets (totaux Power/Toughness vérifiés par script), tables « Total Wish output » (231 souhaits vérifiés par script). Wandoos, Time Machine et Energy / Magic / Resource 3 sont faits (voir plus haut).
