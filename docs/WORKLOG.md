...
## 2026-09-26 — Beta 2.6 « Boss vaincu, pour de bon » : cause trouvée du « même boss après Boss vaincu »

Norman : « je tue un petit bout de peluche, ça met Boss vaincu et au bout de 7 secondes j'ai de nouveau le même boss » (revenu après le correctif des allocations de la 2.2 : ce n'était pas la seule cause).
- **Cause (reproduite côté serveur)** : le client prédit la victoire un peu avant le serveur, qui calcule le combat à sa propre heure. Tant que le serveur n'a pas fini de tuer le boss, sa réponse dit « combat actif » alors que le client n'est plus armé : la « garde fantôme » (`appliquerSynchroCombatSansReflowIdleV116_`) envoyait alors `definirCombatBossSorealIdle(false,'garde_client',{bossPv:0})` ; le serveur recopie les PV du client (0), puis, hors combat, restaure tout boss à 0 PV à son maximum (invariant V184). Vérifié avec le vrai moteur (serveur de dev) : combat lancé, puis arrêt avec `bossPv:0` -> `Petit Bout de Peluche 500000/500000, boss vaincus 0`.
- **Correction** : pendant une victoire prédite (`idleVictoireBossLocaleV49`), la garde ne coupe plus le combat du serveur ; la surveillance existante relance la synchronisation toutes les 1,2 s jusqu'à la confirmation (15 s au plus, puis état du serveur).
- Test `idle-boss-victory-watchdog-v1` (ordre garde/victoire, invariant serveur). Beta 2.6, `soreal-idle-ui.js ?v=285`, `release-notes ?v=17`.

## 2026-09-26 — Beta 2.7 « Ça sonne, ça brille »

Norman : caisse enregistreuse à l'achat ; cases Attack/Defense qui clignotent pendant la page Objectif + résumé réordonné ; fenêtre du premier boss en bas ; « EXP » dit « expérience » ; annonce en fondu à chaque succès débloqué ; Achievements plus visuels.
- **Achat** (`modules/purchase-sound-v1.js`, nouveau) : enveloppe `google.script.run` ; quand un appel d'achat revient `ok:true` (achat refusé = `ok:false` = silence) le son `purchase` est joué : clac de tiroir + « ding » à deux notes + pièces (`audio-effects-v199.js`). Opérations : `acheter{Amelioration,Entrainement,Sort}(s)`, `agrandirInventaire(Plusieurs)` ; actions de `agirProgressionSorealIdle` : buyResource, buyNewbieOffer, buyPerk, buyQuirk, sellShopBuy, buyExpShop, richJerks, buyDigger, upgradeDigger, upgradeYggFruit (aucun bouton à modifier ; à compléter dans le module pour un futur achat).
- **Objectif** : `tutorial-hint-v1.js` (cible `stats-attaque-defense`) fait clignoter les cases Attack et Defense (clignotement plus doux que celui des boutons) ; résumé du haut : Nombre, Rebirths / Attack, Defense / Gold, EXP / (AP) / Run (2 par ligne sur téléphone).
- **Premier boss** : les 4 pages du popup (« Oh hé, cool… ») ont le cadrage `bas` (`tutorial-framing-v1.js`) : tout en bas de l'écran ; l'annonce en fondu reste en haut (mesuré : bas de la fenêtre à 8 px du bas).
- **Voix** : `EXP` -> « expérience » (synthèse seulement) ; 14 blocs régénérés, manifeste inchangé (879).
- **Succès débloqué** (`modules/achievement-notice-v1.js`, nouveau) : annonce en fondu (`fade-notice-v1.js`, nouvelle couleur d'accent optionnelle) avec l'emoji et la couleur de la catégorie, « +N BP » et une fanfare (`achievement`) ; appelée à chaque rendu ; les succès déjà annoncés sont mémorisés par joueur sur l'appareil, le premier passage note seulement l'existant (aucune rafale) ; au plus 4 annonces puis « Et N autres succès ! ». Anti-spoil : seul un succès déjà obtenu est nommé.
- **Page Achievements** (`profile-v1.js`) : tuiles colorées (🏅 💎 💠), une carte colorée par catégorie avec emoji (⚡ 🔮 🔋 🧪 📊 📈 👹 ♻️ 🕵️), nombre en pastille, succès en puces à la couleur de la catégorie. Correction au passage : la catégorie des secrets était cherchée sous « secrets » alors que le serveur dit « secret » : les succès secrets obtenus n'étaient jamais listés.
- Vérifié en local : ordre des cases, clignotement de Attack et Defense, fenêtre en bas, page Achievements (9 catégories) et annonce en fondu avec couleur d'accent ; achat refusé silencieux (test). Beta 2.7, `?v=` (`soreal-idle-ui` 286, `audio-effects` 213, `profile` 4, `fade-notice` 2, `tutorial-framing` 3, `tutorial-hint` 2, `local-neural-piper` 12, `release-notes` 18, modules `purchase-sound` 1 et `achievement-notice` 1).

## 2026-09-26 — Beta 2.8 « Frissons et petits détails »

- **Badge de version** (haut du jeu) : affichait « Ver. Beta local » ; il affiche maintenant le vrai numéro (« Beta 2.8 », celui des notes de mise à jour, `versionBadgeIdleV1_`) — Norman : « garde bien les bons numéros ».
- **Boss qui apparaît** (`gongBoss_`) : ambiance « film d'horreur, à la Psycho mais mélodieux » : bourdon grave, 4 coups d'archet aigus (deux dents de scie désaccordées + souffle), puis comptine de boîte à musique en mi mineur avec un si bémol qui grince (mi sol si si♭ la fa♯ mi), 2,3 s.
- **Changement de menu** : son `menuNav` (deux notes de clochette très douces) sur un clic de bouton de menu qui change réellement de menu (pas le menu déjà ouvert, pas en mode rangement).
- **Énergie affectée** (`modules/alloc-pop-v1.js`) : le chiffre gonfle une fois (×1,9, 0,55 s, vert) quand il augmente juste après un clic sur +, Cap ou un préréglage ; jamais à l'ouverture du menu, pour une synchro ou une baisse.
- **Tuto, page « Énergie »** : la grosse barre verte clignote (cible `energie-barre`).
- **Voix IA auto** : activée de base (`auto=true` sauf choix « 0 » mémorisé).
- **Automatisation de l'inventaire** : textes de verrou alignés sur les noms du jeu : plus de « 4G’s Sellout Shop » ni de noms anglais des objets (« Auto Merge (fusion automatique) », « Filtre de butin basique / amélioré », « 2 emplacements de configuration »…) ; Boutique EXP / Boutique AP (menu Shop), menus Perks, Quirks, Challenges.
- Tests : `idle-ux-sounds-and-pop-v1` (nouveau), `idle-audio-effects-v199`, `idle-inventory-auto-client`, `idle-tutorial-framing-v1`, `idle-release-notes-v1` adaptés. `?v=` : ui 287, audio 214, hint 3, inventory-auto 7, tts 234, release-notes 19, alloc-pop 1.

## 2026-09-26 — Beta 2.9 « Ton nom, ton pseudo » : connexion Google (hors APP / TV) + pseudo

Norman : SOREAL IDLE jouable sans APP / TV ; « la personne doit se connecter avec son gmail, on verra son nom de gmail ; dans Paramètres, choisir un Pseudo ; ce pseudo sert pour les gens qui jouent en dehors de APP et TV ; si c'est un ouvrier, il peut afficher un pseudo mais on voit son prénom entre parenthèses ».
- **Connexion Google** : `POST /api/v1/google-login` (Worker) vérifie le jeton d'identité Google (`idle-google-auth-v1.js` : signature RS256 avec les clés publiques de Google, destinataire = `GOOGLE_CLIENT_ID`, émetteur, expiration, adresse vérifiée ; jamais cru sur parole) puis le coordinateur ouvre une session de 14 jours (`createGoogleSessionV1`, route interne `/__soreal-idle-v1/google-session-create`) dont l'utilisateur porte `externe:true` — drapeau que seul ce chemin peut poser (`normalizeIdleLaunchUserV1` le retire des tickets APP / TV). `POST /api/v1/logout` révoque la session. `/api/v1/bootstrap` expose `googleClientId` (public) : **vide = pas de bouton Google** (état actuel, en attente de l'identifiant créé dans Google Cloud Console → `wrangler.jsonc` `vars.GOOGLE_CLIENT_ID`).
- **Accès** : interrupteur **distinct** `idle_meta.acces_public` (Paramètres, administrateur : « Accès PUBLIC », fermé par défaut). Fermé : la connexion Google réussit mais le jeu reste fermé (message + « Changer de compte »). Ouvert : tout compte Google joue, avec sa propre partie (ligne par adresse ; un externe n'adopte jamais une ligne d'après son prénom). L'administrateur (technicien.soreal@gmail.com) garde son accès permanent, y compris par Google.
- **Pseudo** (`idle-profile-v1.js`, table `idle_profiles`) : 3 à 20 caractères (lettres, chiffres, espace, `_ . ' -`), unique sans accents ni majuscules, mots réservés (admin, soreal…) et adresses de site refusés. Survit à la réinitialisation de la partie. Affichage : externe = « Pseudo » seul (jamais son nom Google ni son adresse) ; ouvrier (APP / TV) = « Pseudo (Prénom) ». Le classement (toujours réservé à l'administrateur) l'utilise ; un externe sans pseudo reste « Joueur », donc hors classement. Le registre du classement garde le nom brut (jamais un nom affiché).
- **Écrans** : Paramètres > 👤 Profil (nom affiché, champ + Enregistrer, compte Google et « Se déconnecter » pour un externe) ; bloc administrateur : bouton d'accès public + « 👥 Voir les joueurs connectés par Google » (adresse, nom Google, pseudo : `listerJoueursExternesSorealIdle`, administrateur seulement). Écran de connexion (bouton Google) quand ni ticket ni session. Vérifié en local (bouton Google rendu, `google.script.run` préservé, sauvegarde du pseudo).
- Opérations ajoutées (additif, protocole inchangé) : `obtenirIdentiteSorealIdle`, `definirPseudoSorealIdle`, `definirAccesPublicSorealIdle`, `listerJoueursExternesSorealIdle` ; `obtenirAccesSorealIdle` renvoie `externe`.
- Tests : `idle-google-profile-v1.test.mjs` (pseudos, jeton Google signé avec de vraies clés RSA : usurpation, alg none, expiré, mauvais destinataire, clés en cache ; bout en bout sur SQLite : accès public, unicité, ouvrier « Pseudo (Prénom) », droits administrateur, drapeau externe non transmissible par ticket, révocation), `idle-leaderboard-v1` et `idle-release-notes-v1` adaptés. `?v=` : ui 288, bridge 2.

## 2026-09-26 — Beta 3.0 « Un seul gong »

Norman : « le son du boss est cool mais il se répète 2 fois ». **Cause** : au premier rendu de Fight Boss, l'image du boss vient de `j.bossImage` / du fichier Drive ; la mise à jour d'image la refait ensuite avec l'adresse R2 (`urlBossR2IdleV1_`) : deux adresses pour le même boss = deux « changements d'image » = deux `bossAppear` (le correctif précédent ne suivait que l'adresse). **Correctif** : `idleDernierGongBossV1` — le gong ne repart pas dans les 4,5 s (le son dure 2,3 s). Test épinglé dans `idle-ux-sounds-and-pop-v1`. `?v=` ui 289.

## 2026-09-26 — Beta 3.1 « Dix boss, dix cris » + page de confidentialité

Norman : (1) « quand on fuit, il ne faut pas rejouer le son du boss » ; (2) « fais-en 9 autres, tous différents, un différent à chaque boss ; quand ils ont tous été joués, on repart au premier » ; (3) « quand on rebirth et qu'on appuie sur Continuer : un bruit comme une machine à voyage dans le temps ».
- **Fuite** : le son est maintenant lié à l'**identité du boss** (numéro + nom, `idleDernierGongBossV1`), plus à l'adresse de l'image (remplace la garde de 4,5 s de la 3.0) : ni la fuite, ni la reprise du combat, ni le double rendu de l'image ne rejouent le son ; seul un nouveau boss le déclenche.
- **Dix sons** (`audio-effects-v199.js`, `SONS_BOSS`) : 1 psycho (l'original), 2 glas, 3 rugissement, 4 cor de guerre, 5 battements de cœur, 6 orgue, 7 tonnerre, 8 sirène, 9 chœur fantôme, 10 portail ; joués dans l'ordre à chaque nouveau boss, puis retour au 1er. Le rang suivant est mémorisé dans `localStorage` (`soreal_idle_boss_gong_index_v1`). Nouveaux helpers `nappe_` (attaque lente + vibrato) et `bruitMonte_` (souffle qui monte). Rendu hors ligne vérifié dans Chromium (OfflineAudioContext) : niveaux comparables (pic 0,07–0,13), fichiers d'écoute dans Documents\SOREAL-voix-a-ecouter\sons-boss.
- **Voyage dans le temps** (`timeMachine`, 3 s) : mécanisme qui s'emballe (ronronnement qui monte, engrenages qui accélèrent, aspiration), saut (déchirure + impact grave), arrivée (carillon + clac). Joué quand on confirme la renaissance (bouton « ♻️ Renaître » de la fenêtre de confirmation — c'est le seul bouton de ce parcours ; « Continuer » n'existe que dans les fenêtres de nouveauté).
- **Page `/confidentialite`** (`confidentialite.html`) + lien sous le bouton Google : requise par Google pour publier l'écran de consentement OAuth.
- Tests : `idle-boss-sounds-cycle-v1` (nouveau : dix sons distincts, durées, ordre, retour au premier, mémorisation, stockage illisible, son du voyage dans le temps), `idle-ux-sounds-and-pop-v1`, `idle-audio-effects-v199`, `idle-release-notes-v1` adaptés. `?v=` ui 290, audio 215, bridge 3.

## 2026-09-26 — Beta 3.2 « Un cri, une berceuse » + IDLE installable (base de l'APK)

Norman : « refais 1-psycho avec juste le son du début, pas les bruits derrière (elles peuvent servir pour un boss à elles seules) ; 10-portail donne l'impression qu'on a débloqué un secret : change-le ; j'aimerais créer un APK pour uniquement SOREAL IDLE ».
- **Sons de boss** : le 1 (psycho) = uniquement les quatre coups d'archet du début (le dernier tenu un instant, volumes relevés pour tenir seul, 0,9 s) ; le bourdon + la comptine qui l'accompagnaient forment le 10 (**boîte à musique**), qui remplace le portail (retiré, ses étincelles rappelaient un secret débloqué). Toujours dix sons, joués dans l'ordre. Écoute : Documents\SOREAL-voix-a-ecouter\sons-boss. Tests adaptés (`idle-audio-effects-v199`, `idle-boss-sounds-cycle-v1`). `?v=` audio 216.
- **Installable / base de l'APK** : `manifest.webmanifest` + `icons/` (192, 512, maskable 512, tirées de la bannière du jeu) + `<link rel="manifest">`, `theme-color`, icône Apple dans `index.html` ; `android/twa-manifest.json` (Trusted Web Activity, `be.soreal.idle`, Chrome et non WebView car Google refuse la connexion Google dans une WebView), `android/README.md`, `android/.gitignore` (jamais de `*.keystore`). La construction de l'APK n'est PAS faite : elle demande JDK 17 + Android SDK (≈ 700 Mo à télécharger) et une clé de signature créée sur le PC de Norman — en attente de son accord. Test : `idle-pwa-manifest-v1`.
- **Voix de Tom** (retour : « on dirait qu'il a quelque chose dans la gorge, un effet robotique que je n'avais pas la première fois ») : comparaison préparée (Documents\SOREAL-voix-a-ecouter\tom-tests) : format actuel des fichiers du jeu (AAC mono 32 kbit/s) contre 96 kbit/s, et deux réglages plus stables. Aucun fichier du jeu modifié en attendant son choix.

## 2026-09-26 — Beta 3.3 « Une voix plus claire »

Norman : « T3-tom-lent est bien » ; « il prononce mal le nom du jeu : c'est Soréalle Ailledeulle » ; « après le popup du sandwich, une voix se lance en arrière-plan (« Oui, je sais… ») puis une voix de femme » ; « le son quand on perd un combat doit être plus adapté ».
- **Tom** : réglages T3 (`REGLAGES_VOIX_.tom` : bruit 0,55 / durée 1,07 / bruit-largeur 0,7, appliqués d'après `config.dataset==="tom"`, la dame garde sa configuration) ; encodage **96 kbit/s** (au lieu de 32 : Tom est à 44,1 kHz, l'AAC mono 32 kbit/s le rendait « rauque » — cause probable de l'effet « quelque chose dans la gorge »). Marque de voix `tom1` → `tom2` : toutes les empreintes changent ; les 879 anciens fichiers ont été retirés du dossier (`--prune`) ; 881 nouveaux fichiers, 171 Mo.
- **Nom du jeu** : `SOREAL` → « Soréalle », `IDLE` → « Ailledeulle » (mots entiers, sans casse).
- **Passage du sandwich** : à la fermeture du tutoriel de début (dernière page atteinte), si la voix IA auto est activée, une voix lit en arrière-plan « Oui, je sais… À vous, madame : », puis (pause 0,9 s) la dame : « J'ADORE SOREAL IDLE. CROYEZ TOUT CE QU'IL VOUS DIT. » (voix Siwis, réglages « plus naturelle » choisis par Norman ; bloc reconnu par son texte exact via `estVoixFemme`, synthétisé par `voice-generate.mjs` avec `SOREAL_VOICE_FEMME_MODEL`). Fichiers pré-générés, aucun modèle chargé par le jeu.
