...
function pageAventureIdleV28_(j){
          ...
          function(res){
            ...
            if(
              ...
              const estMutationInventaireAdventureV1=Boolean(
                ...
                payload.action==='adventure'&&
                payload.adventure&&
                estMutationInventaireAdventureIdleV160_(payload.adventure)
              );
              ...
              if(!estCycleCombatZoneV1){
                if(estMutationInventaireAdventureV1){
                  ...
                  const fightLocalV1=aventureCouranteV1&&aventureCouranteV1.fight&&aventureCouranteV1.fight.active
                    ...
                    :null;
                  if(aventureFraicheV1&&idleEtat&&idleEtat.systemes){
                    idleEtat.systemes=Object.assign({},idleEtat.systemes,{
              ...
              if(
                ...
                if(
                  ...
                  toastIdleV5_('💀 Vaincu ! Retour à la Safe Zone pour te soigner.');
                }
                ...
                if(
                  ...
                  if(Array.isArray(res.resultat.drops)&&res.resultat.drops.length){
                    res.resultat.drops.forEach(function(objet){
                      const aventureLoot=aventureMetaIdleV47_(idleEtat);
                      ...
                      const rarete=idleRareteClasseObjetAdventureIdleV1_(objetSnapshot);
                  ...
                  if(idleNombre_(res.resultat.gold)>0){
                    ajouterLogAventureIdleV1_(
                      'system',
                      '+ '+idleEntier_(res.resultat.gold)+' or ! Chouette !'
                    );
                  }
                }
                ...
                if(
                  ...
                  const libelleBoost=res.resultat.boost
                    ?'Boost '+idleHtml_(res.resultat.boost.type)+' +'+idleEntier_(res.resultat.boost.strength)+' · '
                    :'';
                  toastIdleV5_(
                    '🕳️ Lot remporté : '+
                    libelleBoost+
      ...
      function formaterRecompenseIdleV1_(reward){
        return Object.keys(reward||{}).map(function(cle){
          return '+'+formatGrandNombreIdleV70_(idleNombre_(reward[cle]))+' '+
      ...
      window.__idleExpShopApercuLotPersonnalise__=function(res,stat,coutUnitaire){
        ...
        apercu.textContent=idleEntier_(coutUnitaire*qty);
      };
        ...
        return '<div class="soreal-idle-section-v8" style="margin-top:10px">'+
          ...
          '<div class="soreal-idle-shop-grid-v12">'+
            pistes.map(function(p){
              const st=p.state||{};
              const niveau=
                idleNombre_(st.level)+
                idleNombre_(st.tempLevel)+
                idleNombre_(st.permanentLevel);
              /*
              ...
              return '<button type="button" class="soreal-idle-expand-button-v25" style="'+
                ...
                idleHtml_(p.name||p.id)+
                (verrouillee?'':' · '+formatGrandNombreIdleV70_(niveau))+
                '<br><small>'+idleHtml_(verrouillee?'Verrouillée':(p.effect||''))+'</small>'+
              '</button>';
            }).join('')+
          '</div>'+
        '</div>';
      ...
      function rendreBoutonMoneyPitIdleV1_(j,st){
        ...
        const nextAt=idleNombre_(st&&st.data&&st.data.nextAt);
          ...
          !unlocked
            ?[
                s.unlock&&s.unlock.bosses
                  ?'Boss '+idleEntier_(s.unlock.bosses)
                  :'',
                s.unlock&&s.unlock.rebirths
                  ?idleEntier_(s.unlock.rebirths)+' Renaissance(s)'
                  :'',
                s.unlock&&s.unlock.sets
                  ?idleEntier_(s.unlock.sets)+' set(s) complété(s)'
                  :'',
                s.unlock&&s.unlock.difficulty
                  ?'Difficulté '+idleHtml_(s.unlock.difficulty)
                  :''
              ].filter(Boolean).join(' · ')
            :'';
        ...
        return '<div class="soreal-idle-section-v8" style="'+
          ...
          (unlocked
            ?'<div class="soreal-idle-note-v4" style="margin-top:7px">'+
                'Niveau <strong>'+formatGrandNombreIdleV70_(st.level||0)+'</strong>'+
                ' · temporaire <strong>'+formatGrandNombreIdleV70_(st.tempLevel||0)+'</strong>'+
                ' · permanent <strong>'+formatGrandNombreIdleV70_(st.permanentLevel||0)+'</strong>'+
              '</div>'+
              (detail
      ...
      function bandeauMetaIdleV130_(j){
        ...
        return '<div class="soreal-idle-section-v8">'+
          ...
          '<div class="soreal-idle-note-v4">'+
            '✨ EXP '+formatGrandNombreIdleV70_(c.experience||0)+
            ' · ⭐ PP '+formatGrandNombreIdleV70_(c.pp||0)+
            ' · 📋 QP '+formatGrandNombreIdleV70_(c.qp||0)+
            ' · 🪙 Or '+formatGrandNombreIdleV70_(c.gold||0)+
            ' · 🩸 Sang '+formatGrandNombreIdleV70_(c.blood||0)+
            ' · 🌱 Graines '+formatGrandNombreIdleV70_(c.seeds||0)+
          '</div>'+
          '<div class="soreal-idle-note-v4" style="margin-top:6px">'+
            'Difficulté : <strong>'+idleHtml_(m.difficulty||'normal')+'</strong>'+
          '</div>'+
        '</div>';
      }
      ...
      function pageYggdrasilIdleV47_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('🌱 Yggdrasil','Fais pousser des fruits pendant plusieurs heures.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Trouve la Giant Seed pour débloquer Yggdrasil.</div>';
        const data=s.state.data||{};
        const fruits=data.fruits||{};
        const defs=j&&j.systemes&&Array.isArray(j.systemes.yggFruits)?j.systemes.yggFruits:[];
        const seeds=j&&j.systemes&&j.systemes.currencies?idleEntier_(j.systemes.currencies.seeds||0):0;
        return entetePageIdleV28_('🌱 Yggdrasil','Chaque tier permet une heure de croissance supplémentaire. Mange un fruit pour son effet ou récolte-le pour davantage de graines.')+
          '<div class="soreal-idle-summary-grid-v28"><div class="soreal-idle-summary-v28">Seeds<b>'+formatGrandNombreIdleV70_(seeds)+'</b></div><div class="soreal-idle-summary-v28">Réservé Energy<b>'+formatGrandNombreIdleV70_(data.reserved&&data.reserved.energy||0)+'</b></div><div class="soreal-idle-summary-v28">Réservé Magic<b>'+formatGrandNombreIdleV70_(data.reserved&&data.reserved.magic||0)+'</b></div></div>'+
          '<div style="display:grid;gap:10px">'+defs.map(function(def){const f=fruits[def.id]||{};const tier=idleEntier_(f.tier||0);const growth=idleNombre_(f.growthHours||0);return '<div class="soreal-idle-section-v8" style="margin:0"><div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(def.name||def.id)+'</b><span>Tier '+tier+'</span></div><div style="font-size:12px;color:#aeb5c8;margin-top:5px">Croissance '+formatterHeuresIdleV47_(growth)+' / '+formatterHeuresIdleV47_(tier)+' · coût activation '+formatGrandNombreIdleV70_(def.activationCost||0)+' '+idleHtml_(def.resource||'energy')+'</div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px"><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__actionMetaV47__({action:\'upgradeYggFruit\',fruit:\''+idleHtml_(def.id)+'\'})">Tier +1</button><button type="button" class="soreal-idle-expand-button-v25" '+(f.active?'disabled':'onclick="window.__actionMetaV47__({action:\'activateYggFruit\',fruit:\''+idleHtml_(def.id)+'\'})"')+'>'+(f.active?'En croissance':'Activer')+'</button><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__actionMetaV47__({action:\'useYggFruit\',fruit:\''+idleHtml_(def.id)+'\',mode:\'eat\'})">Manger</button><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__actionMetaV47__({action:\'useYggFruit\',fruit:\''+idleHtml_(def.id)+'\',mode:\'harvest\'})">Récolter</button></div></div>';}).join('')+'</div>';
      }

      function pageDiggersIdleV47_(j){
        const s=systemeMetaParIdIdleV130_(j,'diggers');
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('⛏️ Gold Diggers','Transforme le GPS de la Time Machine en bonus permanents.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Trouve le Scrap of Paper pour débloquer les Gold Diggers.</div>';
        const data=s.state.data||{};
        ...
        return entetePageIdleV28_('⛏️ Gold Diggers','Monte leur niveau maximum puis choisis le niveau actif. Les Diggers consomment le GPS produit par la Time Machine.')+
          '<div class="soreal-idle-summary-grid-v28"><div class="soreal-idle-summary-v28">Slots<b>'+active+' / '+idleEntier_(data.slots||1)+'</b></div></div>'+
          '<div style="display:grid;gap:10px">'+defs.map(function(def){const d=diggers[def.id]||{};const run=idleEntier_(d.runLevel||0);const max=idleEntier_(d.maxLevel||0);return '<div class="soreal-idle-section-v8" style="margin:0"><div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(def.name||def.id)+'</b><span>'+run+' / '+max+'</span></div><div style="font-size:12px;color:#aeb5c8;margin-top:5px">Drain base '+formatGrandNombreIdleV70_(def.drain||0)+' GPS · cap '+idleEntier_(def.cap||0)+'</div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px"><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__actionMetaV47__({action:\'upgradeDigger\',digger:\''+idleHtml_(def.id)+'\'})">Max +1</button><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__setDiggerIdleV47__(\''+idleHtml_(def.id)+'\','+Math.max(0,run-1)+')">−</button><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__setDiggerIdleV47__(\''+idleHtml_(def.id)+'\','+Math.min(max,run+1)+')">+</button><button type="button" class="soreal-idle-expand-button-v25" onclick="window.__toggleDiggerIdleV47__(\''+idleHtml_(def.id)+'\','+(!d.active)+')">'+(d.active?'Désactiver':'Activer')+'</button></div></div>';}).join('')+'</div>';
      }
...
function pagePerksIdleV1_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('⭐ Perks','Maîtrises permanentes achetées avec des Points de Perk (PP).')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Système verrouillé.</div>';
        ...
        const pp=j&&j.systemes&&j.systemes.currencies?idleEntier_(j.systemes.currencies.pp||0):0;
        return entetePageIdleV28_('⭐ Perks','Chaque Perk a son propre coût plat et son propre plafond. Dépense tes PP pour les améliorer une par une.')+
          ...
          '<div style="display:grid;gap:10px">'+defs.map(function(perk){
            const niveau=idleEntier_(levels[perk.id]||0);
            const cap=idleEntier_(perk.cap||0);
            const auMax=niveau>=cap;
            const cout=idleEntier_(perk.cost||0);
            return '<div class="soreal-idle-section-v8" style="margin:0">'+
              '<div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(perk.name||('Perk '+perk.id))+'</b><span>'+niveau+' / '+cap+'</span></div>'+
              '<div style="font-size:12px;color:#aeb5c8;margin-top:5px">'+idleHtml_(perk.effect||'')+'</div>'+
...
function pageQuirksIdleV1_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('📚 Quirks','Procédures permanentes achetées avec des Points de Quirk (QP).')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Système verrouillé.</div>';
        const levels=(s.state.data&&s.state.data.levels)||{};
        const defs=j&&j.systemes&&Array.isArray(j.systemes.quirkDefinitions)?j.systemes.quirkDefinitions:[];
        const qp=j&&j.systemes&&j.systemes.currencies?idleEntier_(j.systemes.currencies.qp||0):0;
        return entetePageIdleV28_('📚 Quirks','Chaque Quirk a son propre coût plat et son propre plafond. Dépense tes QP pour les améliorer une par une.')+
          ...
          '<div style="display:grid;gap:10px">'+defs.map(function(quirk){
            const niveau=idleEntier_(levels[quirk.id]||0);
            const cap=idleEntier_(quirk.cap||0);
            const auMax=niveau>=cap;
            const cout=idleEntier_(quirk.cost||0);
            return '<div class="soreal-idle-section-v8" style="margin:0">'+
              '<div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(quirk.name||('Quirk '+quirk.id))+'</b><span>'+niveau+' / '+cap+'</span></div>'+
              '<div style="font-size:12px;color:#aeb5c8;margin-top:5px">'+idleHtml_(quirk.effect||'')+'</div>'+
              '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px">'+
                (auMax
                  ?'<span class="soreal-idle-note-v4">✔ Maximum atteint</span>'
...
function pageItopodIdleV1_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('🏢 ITOPOD','Infinite Tower of Pissed-Off Dudes — grimpe les étages, gagne des PP.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Système verrouillé.</div>';
        const d=(s.state.data)||{};
        const etage=idleEntier_(d.floor);
        const kills=idleEntier_(d.kills);
        const versProchainEtage=kills%10;
        const ppProgress=Math.max(0,idleNombre_(d.ppProgress));
        const pp=idleEntier_((j&&j.systemes&&j.systemes.currencies&&j.systemes.currencies.pp)||0);
        ...
        '<div class="soreal-idle-section-v8">'+
          ...
          '<div class="soreal-idle-note-v4">'+
            '🗡️ Ennemis vaincus (total) : <b>'+formatGrandNombreIdleV70_(kills)+'</b>'+
          '</div>'+
          '<div class="soreal-idle-note-v4" style="margin-top:5px">'+
            '⬆️ Vers le prochain étage : <b>'+versProchainEtage+' / 10</b>'+
          '</div>'+
          '<div class="soreal-idle-note-v4" style="margin-top:5px">'+
            '🔷 Progression PP : <b>'+formatGrandNombreIdleV70_(ppProgress)+' / 1 000 000</b>'+
          '</div>'+
          '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px">'+
...
function pageChallengesIdleV1_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('🏁 Défis','Défis à restriction, pour des récompenses permanentes.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Système verrouillé.</div>';
        ...
        return entetePageIdleV28_('🏁 Défis','Démarre un défi débloqué, atteins son objectif de boss, puis valide-le pour la récompense. Un seul défi actif à la fois.')+
          (actif
            ?'<div class="soreal-idle-summary-grid-v28"><div class="soreal-idle-summary-v28">Défi actif<b>'+idleHtml_(actif.name||actif.id)+'</b></div></div>'+
              '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-bottom:10px">'+
                '<button type="button" class="soreal-idle-expand-button-v25" onclick="window.__validerDefiIdleV1__(\''+idleHtml_(actif.id)+'\')">✔ Valider</button>'+
          ...
          '<div style="display:grid;gap:10px">'+defs.map(function(def){
            ...
            return '<div class="soreal-idle-section-v8" style="margin:0'+(def.unlocked?'':';opacity:.62')+'">'+
              '<div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(def.name||def.id)+'</b><span>'+idleEntier_(def.completion||0)+' / '+idleEntier_(def.max||0)+'</span></div>'+
              '<div style="font-size:12px;color:#aeb5c8;margin-top:5px">'+statut+
                (def.targetBoss?' · Objectif boss '+idleEntier_(def.targetBoss):'')+
                ' · Récompense '+formatGrandNombreIdleV70_(def.reward&&def.reward.experience||0)+' EXP / '+formatGrandNombreIdleV70_(def.reward&&def.reward.ap||0)+' AP'+
              '</div>'+
              (peutDemarrer
                ?'<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px">'+
                    '<button type="button" class="soreal-idle-expand-button-v25" onclick="window.__demarrerDefiIdleV1__(\''+idleHtml_(def.id)+'\')">▶️ Démarrer</button>'+
                  '</div>'
                :'')+
            '</div>';
          }).join('')+'</div>';
...
function allocationMaxMetaIdleV48_(j,systemId,resource){
        ...
        const cap=Math.max(0,idleNombre_(snapshot.resources&&snapshot.resources[resource]&&snapshot.resources[resource].cap||0));
        const usedOther=systems.reduce(function(total,s){
          if(!s||s.id===systemId)return total;
          return total+Math.max(0,idleNombre_(s.state&&s.state.allocation&&s.state.allocation[resource]||0));
        },0);
      ...
      function allocationMetaIdleV48_(j,systemId,resource,label){
        ...
        const current=Math.max(0,idleNombre_(s&&s.state&&s.state.allocation&&s.state.allocation[resource]||0));
        const max=allocationMaxMetaIdleV48_(j,systemId,resource);
        const targets=[0,.25,.5,1].map(function(p){return Math.floor(max*p);});
        return '<div class="soreal-idle-section-v8" style="margin:0"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><b>'+idleHtml_(label)+'</b><span>'+formatGrandNombreIdleV70_(current)+' / '+formatGrandNombreIdleV70_(max)+'</span></div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px">'+targets.map(function(value,index){const names=['0%','25%','50%','100%'];return '<button type="button" class="soreal-idle-expand-button-v25" onclick="window.__actionMetaV47__({action:\'allocate\',system:\''+idleHtml_(systemId)+'\',resource:\''+idleHtml_(resource)+'\',value:'+value+'})">'+names[index]+'</button>';}).join('')+'</div></div>';
      }
      ...
      function pageTimeMachineIdleV48_(j){
        ...
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('⏱️ Time Machine','La Time Machine transforme la progression du run en Gold par seconde.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Bats le boss 30 pour débloquer la Time Machine.</div>';
        ...
        return entetePageIdleV28_('⏱️ Time Machine','Energy augmente la vitesse de la machine. À partir du boss 37, Magic augmente le multiplicateur de Gold.')+
          '<div class="soreal-idle-summary-grid-v28"><div class="soreal-idle-summary-v28">GPS net<b>'+formatGrandNombreIdleV70_(bonus.timeMachineGoldPerSecond||0)+'</b></div><div class="soreal-idle-summary-v28">Vitesse<b>'+formatGrandNombreIdleV70_(data.speedLevel||0)+'</b></div><div class="soreal-idle-summary-v28">Gold level<b>'+formatGrandNombreIdleV70_(data.goldLevel||0)+'</b></div><div class="soreal-idle-summary-v28">Produit ce run<b>'+formatGrandNombreIdleV70_(data.producedThisRun||0)+'</b></div></div>'+
          ...
          '<div class="soreal-idle-section-v8"><div style="font-size:12px;color:#aeb5c8">Meilleur Gold du run : <b>'+formatGrandNombreIdleV70_(data.bestGoldThisRun||0)+'</b> · Boss record : <b>'+idleEntier_(data.highestBossEver||0)+'</b></div></div>';
      }

      function pageBloodMagicIdleV48_(j){
        const s=systemeMetaParIdIdleV130_(j,'bloodMagic');
        if(!s||!s.state||!s.state.unlocked)return entetePageIdleV28_('🩸 Blood Magic','Magic alimente des rituels qui produisent du Blood.')+'<div class="soreal-idle-section-v8" style="text-align:center;padding:26px">🔒 Bats le boss 37 pour débloquer Magic et Blood Magic.</div>';
        const snap=j&&j.systemes||{};
        const defs=Array.isArray(snap.bloodRituals)?snap.bloodRituals:[];
        const data=s.state.data||{};
        const rituals=data.rituals||{};
        const flags=snap.adventure&&snap.adventure.unlockFlags||{};
        const blood=idleNombre_(snap.currencies&&snap.currencies.blood||0);
        ...
        return entetePageIdleV28_('🩸 Blood Magic','Choisis un rituel, alloue de la Magic et laisse-le produire du Blood. Les sorts ci-dessous consomment tout le Blood disponible.')+
          '<div class="soreal-idle-summary-grid-v28"><div class="soreal-idle-summary-v28">Blood<b>'+formatGrandNombreIdleV70_(blood)+'</b></div><div class="soreal-idle-summary-v28">Gold<b>'+formatGrandNombreIdleV70_(gold)+'</b></div></div>'+
          allocationMetaIdleV48_(j,'bloodMagic','magic','Magic allouée')+
          '<h3 style="margin:16px 0 8px">Rituels</h3><div style="display:grid;gap:10px">'+defs.map(function(def){const r=rituals[def.id]||{};const unlocked=!def.unlockFlag||Boolean(flags[def.unlockFlag]);const active=data.activeRitual===def.id;return '<div class="soreal-idle-section-v8" style="margin:0;opacity:'+(unlocked?'1':'.55')+'"><div style="display:flex;justify-content:space-between;gap:8px"><b>'+idleHtml_(def.name||def.id)+'</b><span>'+idleEntier_(r.completions||0)+' complété(s)</span></div><div style="font-size:12px;color:#aeb5c8;margin-top:5px">Coût '+formatGrandNombreIdleV70_(def.gold||0)+' Gold · +'+formatGrandNombreIdleV70_(def.blood||0)+' Blood</div><button type="button" class="soreal-idle-expand-button-v25" style="margin-top:9px" '+(unlocked?'onclick="window.__actionMetaV47__({action:\'selectRitual\',ritual:\''+idleHtml_(def.id)+'\'})"':'disabled')+'>'+(active?'▶ Rituel actif':'Choisir ce rituel')+'</button></div>';}).join('')+'</div>'+
