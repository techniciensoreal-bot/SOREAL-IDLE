...
function rareteAleatoireSorealIdle_() {
  const raretes =
    raretesSorealIdle_();
...
function lootEligibleSorealIdle_(
  loot,
  contexte
) {
  const ctx =
  ...
  /*
   * ZoneID 0 = loot générique utilisable par le combat principal.
   * ZoneID > 0 = loot réservé à cette zone d'Aventure.
   */
  if (
    zoneId > 0 &&
    loot.zoneId !== zoneId
...
function choisirLootPondereSorealIdle_(
  loots
) {
  if (
    !Array.isArray(loots) ||
...
function genererObjetLootSorealIdle_(
  ...
  const eligibles =
    lootsSorealIdle_()
      .filter(function(loot) {
        return lootEligibleSorealIdle_(
          loot,
          ctx
        );
      });
  ...
  /*
   * Chaque ligne peut avoir sa propre chance de drop.
   * Si aucun objet ne réussit son jet, aucun loot n'est donné.
   */
  const multiplicateurChance =
    Math.max(
      0,
      nombreSorealIdle_(
        ctx.chanceMultiplicateur,
        1
      )
    );

  const reussis =
    eligibles.filter(function(loot) {
      return (
        Math.random() <
        Math.max(
          0,
...
function genererObjetLegendaireRareAventureSorealIdle_(
  ...
  const chance =
    Math.max(
      ...
      Math.min(
        1,
        nombreSorealIdle_(
          monstre.chanceLegendaire,
          parametreSorealIdle_(
            'AVENTURE_RARE_LEGENDAIRE_CHANCE',
            0.25
          )
        )
      )
  ...
  const slotBrut =
    String(
      monstre.slotLegendaire || ''
    )
      .trim()
      .toLowerCase();

  const slot =
    slots.indexOf(slotBrut) !== -1
      ? slotBrut
      : 'bijou1';
  ...
  /*
   * L'objet suit légèrement le niveau actuel, mais seulement jusqu'à
   * +25 %. Un coup de chance ne doit jamais sauter plusieurs mondes.
   */
  const surNiveau =
    Math.max(
      0,
...
function definirCombatBossSorealIdle(
  ...
  try {
    ...
    if (!stats.combatBossActif) {
      ...
      /*
       * Norman (2026-09-11) : "Quand on prend la fuite contre les boss...
       * on récupère très vite sa vie. Il faut que le régén se fasse
       * lentement comme c'est prévu dans NGU." Cause : cette fonction ne
       * touche jamais DERNIERE_SYNCHRO. appliquerProgressionEnergieSorealIdle_
       * (le calcul de régén hors-combat, ~2,5%/s) applique son taux à TOUT
       * le temps écoulé depuis la dernière valeur de DERNIERE_SYNCHRO — qui,
       * pendant un combat simulé côté client, ne date que de la dernière
       * synchro périodique (jusqu'à ~15s plus tôt, jamais mise à jour par
       * les coups portés localement). Sans ce timbre, la prochaine lecture
       * d'état après une fuite crédite donc jusqu'à ~15s de régén "hors
       * combat" pour une période où le joueur combattait activement — un
       * bond de vie visible au lieu d'une remontée lente. Marquer
       * DERNIERE_SYNCHRO à l'instant de la fuite fait démarrer le calcul
       * de régén exactement à ce moment, jamais avant.
       */
      feuille
        .getRange(
          ligne,
...
function obtenirImageReposSorealIdle(
  sessionToken,
  numero
) {
  exigerAccesSorealIdle_(
  ...
  return {
    ...
    image:
      imageReposParNumeroSorealIdle_(
        numero
      )
  };
}
...
function obtenirImageJoueurSorealIdle(
  sessionToken,
  numero
) {
  exigerAccesSorealIdle_(
  ...
  return {
    ...
    image:
      imageJoueurParNumeroSorealIdle_(
        numero
      )
  };
}
...
function obtenirImageBossSorealIdle(
  sessionToken,
  nomBoss
) {
  const acces =
...
function acheterAmeliorationsSorealIdle(
  ...
  const source=
    demandes&&typeof demandes==='object'
      ?demandes
      :{};
  ...
  try{
    ...
    const bonusCapaciteOrganisation=
      Math.max(
        0,
        organisationEnergie-1
      )*
      Math.max(
        0,
        nombreSorealIdle_(
    ...
    const multiplicateurProdOrganisation=
      1+
      Math.max(
        0,
        organisationEnergie-1
        ...
        Math.round(
          (
            CONFIG_SOREAL_IDLE
              .ENERGIE_MAX_BASE+
            bonus.capacite+
            bonusCapaciteOrganisation
          )*
          bonusRenaissance
            .multiplicateurCapacite
        )
      );
      ...
      Math.max(
        ...
        Math.round(
          (
            ...
            multiplicateurProdOrganisation*
            bonusRenaissance
...
/**
 * Achat d'un niveau d'entraînement.
 *
 * Le client transmet uniquement le type demandé.
 * Le serveur :
 * - recalcule l'énergie gagnée depuis la dernière synchro ;
 * - relit le niveau actuel ;
 * - calcule lui-même le coût ;
 * - refuse l'achat si l'énergie est insuffisante ;
 * - débite l'énergie ;
 * - augmente la statistique ;
 * - recalcule la puissance depuis CONFIG ;
 * - renvoie l'état complet.
 */
function appliquerAchatsEntrainementsSorealIdle_(
  feuille,
  ligne,
  demandes
) {
  ...
  /*
   * On écrit uniquement les statistiques réellement modifiées.
   * La puissance est ensuite recalculée par LA fonction centrale,
   * qui inclut équipement, sets, collection, boutique et renaissance.
   */
  feuille
    .getRange(
      ligne,
  ...
  const inventaireMaj =
    parserJsonSorealIdle_(
      feuille.getRange(ligne,c.INVENTAIRE_JSON).getValue(),
      []
    );
  const equipementMaj =
    parserJsonSorealIdle_(
      feuille.getRange(ligne,c.EQUIPEMENT_JSON).getValue(),
      {}
    );
  const pvMaxMaj =
    pvMaxJoueurSorealIdle_(
      endurance,
      inventaireMaj,
      equipementMaj
    );
  const pvActuelsMaj =
    Math.min(
      ...
      Math.max(
        0,
        nombreSorealIdle_(
          feuille.getRange(ligne,c.PV_JOUEUR).getValue(),
          pvMaxMaj
        )
      )
    );
  feuille.getRange(ligne,c.PV_JOUEUR_MAX).setValue(pvMaxMaj);
  feuille.getRange(ligne,c.PV_JOUEUR).setValue(pvActuelsMaj);
...
function acheterAmeliorationSorealIdle(
  ...
  if (!lock.tryLock(5000)) {
    return {
      ...
      code:
        'SOREAL_IDLE_OCCUPE',
      message:
        'Le jeu est occupé. Réessaie dans un instant.'
    };
  }
  ...
  try {
    ...
    const bonusCapaciteOrganisationBoutique =
      Math.max(0,organisationBoutique - 1) *
      Math.max(
        0,
        nombreSorealIdle_(
          parametreSorealIdle_(
    ...
    const multiplicateurProdOrganisationBoutique =
      1 +
      Math.max(0,organisationBoutique - 1) *
      Math.max(
        0,
        ...
        Math.round(
          (
            CONFIG_SOREAL_IDLE
              .ENERGIE_MAX_BASE +
            bonus.capacite +
            bonusCapaciteOrganisationBoutique
          ) *
          bonusRenaissance
            .multiplicateurCapacite
        )
      ...
      Math.max(
        ...
        Math.round(
          (
            ...
            multiplicateurProdOrganisationBoutique *
            bonusRenaissance
              .multiplicateurProduction
          ) *
          1000
...
function dossierBossDriveSorealIdle_() {
  return String(
    parametreSorealIdle_(
      'DOSSIER_BOSS_DRIVE_ID',
      '1nHuHi1KY-rVp3oIHla4iFmi937KTA_Gw'
    )
  ).trim();
}
...
function imageBossParNomSorealIdle_(nomBoss) {
  const nom =
    String(nomBoss || '').trim();
  ...
  if (
    definition &&
    definition.driveFileId
  ) {
    const direct =
      imageDriveParIdSorealIdle_(
        definition.driveFileId
      );
...
function imageDriveParIdSorealIdle_(fileId) {
  const id =
    String(fileId || '').trim();
...
function imageFichierDriveSorealIdle_(
  dossierId,
  nomFichier
) {
  const dossier =
  ...
  try {
    ...
    return imageDriveParIdSorealIdle_(
      files.next().getId()
    );
  } catch (e) {
    return '';
...
function imageReposParNumeroSorealIdle_(numero) {
  const n =
    Math.max(
      1,
      Math.floor(
  ...
  const idDirect =
    ...
    definition.driveFileId
      ...
      : (
          n === 1
            ? '18ExcEYOJxt8ge8s_IlsTbcdrlP7bn1rG'
            : ''
        );
  ...
  return imageFichierDriveSorealIdle_(
    parametreSorealIdle_(
      'DOSSIER_REPOS_DRIVE_ID',
      '1TjqooGP07q_2yOtzYvzs8ZTLAj_LISWL'
    ),
    (
      definition &&
      definition.image
...
function imageJoueurParNumeroSorealIdle_(numero) {
  const n =
    Math.max(
      1,
      Math.floor(
  ...
  const idDirect =
    ...
    definition.driveFileId
      ...
      : (
          n === 1
            ? '1KuCRp3P8aHtb2jskJI8JpVCPdSY7VLCV'
            : ''
        );
  ...
  image =
    imageFichierDriveSorealIdle_(
      parametreSorealIdle_(
        'DOSSIER_JOUEUR_DRIVE_ID',
        '1Skqj3tOM8G23LDp3Xg2gLQQwSSMm420D'
      ),
      (
        definition &&
        definition.image
...
/* Le Worker ne lit plus les octets d'image via DriveApp. */
imageDriveParIdSorealIdle_ = function(fileId){
  const id=String(fileId||"").trim();
  return id
    ?"https://drive.google.com/thumbnail?id="+encodeURIComponent(id)+"&sz=w1200"
    :"";
};
imageFichierDriveSorealIdle_ = function(){return "";};
...
const IDLE_OPERATIONS={
  ...
  obtenirImageBossSorealIdle,
  obtenirImageJoueurSorealIdle,
  obtenirImageReposSorealIdle,
